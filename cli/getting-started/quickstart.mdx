...
`testsprite setup` prompts for your API key, verifies it, and installs the TestSprite agent skills (verification loop + onboarding) into your agent's project configuration. Run it once per project.
...
.claude/skills/testsprite-onboard/SKILL.md  →  written
```
...
<Card title="Agent Integration" href="/cli/core/agent-integration" icon="robot">
  All five supported agent targets (claude, cursor, cline, antigravity, codex) and how the skills work
</Card>
