...
There are two ways to use the TestSprite CLI, and both start with the same one-minute install. Most users let their **coding agent** — Claude Code, Codex, Cursor, Cline, Windsurf, Copilot — drive the CLI: you describe what you want in plain language, and the agent creates tests, triggers real cloud runs, reads what broke, fixes the code, and reruns. Prefer typing commands yourself? Switch to the **terminal** tab below.
...
Run it once from your repo root and paste your API key when prompted (create one in the Portal under <kbd>Settings → API Keys</kbd> — see [API Keys](/web-portal/admin/api-keys)). You're set when you see:
...
The CLI ships two **agent skills** — the instruction manual your coding agent follows: when to run tests, how to read failure bundles, and how to loop until green. Every prompt below is powered by them, so take five seconds to install them for the agent you use. Run the command from the root of the repo you're working on — that's where your agent looks for them:
...
See [Coding Agent Integration](/cli/core/agent-integration#supported-agents) for all supported agents and where each skill file lands.
...
2. **Ask you for your deployed app URL** — and login credentials, if your flows need them. Have these ready; it's the only thing the agent can't figure out from your code.
