...
<Steps>
  <Step title="Install and onboard">
    Install once globally, then run `testsprite setup`. It prompts for your API key, verifies it against the platform, and installs the agent skills into your agent's project — so your agent already knows when and how to run tests. Skills install for Claude Code by default; pass `--agent codex`, `--agent cursor`, etc. for a different agent.
...
| :--- | :--- |
| <kbd>AI coding agents</kbd> <br/> _Primary_ | Claude Code, Codex, Cursor, Cline, Antigravity, Windsurf, Copilot, and more — the agent drives the loop without a human in the middle. |
