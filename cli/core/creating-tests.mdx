...
TestSprite stores and executes all test code as Python: frontend tests run as async Playwright scripts, backend tests as `requests` + pytest-style assertions. Accordingly, `test code put --language` accepts only `python`.
